...
// Open Targets (https://platform.opentargets.org) is a public biomedical
// knowledge graph platform integrating evidence linking gene targets to
// diseases and drugs. We use its GraphQL API to ground entity extraction and
// relation inference in real, current target/disease/drug associations rather
// than relying solely on Claude's training data.
//
// Reference: Buniello et al., "Open Targets Platform: facilitating
// therapeutic hypotheses building in drug discovery", Nucleic Acids Research
// 2024-2025 (https://doi.org/10.1093/nar/gkae1128). Platform releases are
// continuously published in the NAR Database Issue.
...
/**
 * Search Open Targets for a free-text term. Returns up to `limit` hits across
 * all entity types (target, disease, drug). Each hit carries id, name, entity
 * type, and a relevance score.
 *
 * Fail-soft: any error returns [] so the pipeline keeps running.
 */
async function openTargetsSearch(term, limit = 3) {
  ...
  try {
    const res = await fetch(OPENTARGETS_GRAPHQL, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "User-Agent": "kg-module3/1.0 (research)"
      },
      body: JSON.stringify({ query, variables: { queryString: term } }),
      signal: AbortSignal.timeout(12000)
    });

    if (!res.ok) {
      process.stderr.write(`[kg-module3] OpenTargets search HTTP ${res.status} for "${term}" — skipping\n`);
      return [];
    }
    ...
    return hits.slice(0, limit).map(h => ({
      id: h.id,
      name: h.name,
      entity: h.entity,    // "target" | "disease" | "drug"
      score: h.score
    }));
  } catch (err) {
    process.stderr.write(`[kg-module3] OpenTargets search error for "${term}": ${err.message} — skipping\n`);
    return [];
  }
}

/**
 * Resolve a list of free-text entity names to Open Targets hits in one batch.
 * Returns { entityName → [hits] } with empty arrays for misses or errors.
 */
async function openTargetsResolveEntities(names, limit = 3) {
  const out = {};
  if (!Array.isArray(names) || names.length === 0) return out;

  for (const n of names) {
    if (typeof n !== "string" || !n.trim()) continue;
    out[n] = await openTargetsSearch(n, limit);
  }
  return out;
}

/**
 * Format Open Targets hits as a compact prompt block for Claude. Returns ""
 * if there are no hits so the prompt cleanly degrades.
 */
function formatOpenTargetsContext(hitsByEntity) {
  if (!hitsByEntity || Object.keys(hitsByEntity).length === 0) return "";

  const lines = [];
  for (const [entity, hits] of Object.entries(hitsByEntity)) {
    if (!hits || hits.length === 0) continue;
    const summary = hits.slice(0, 2).map(h =>
      `${h.name} [${h.entity}:${h.id}]`
    ).join(" | ");
    lines.push(`  "${entity}" → ${summary}`);
  }

  if (lines.length === 0) return "";
  return `\nOpen Targets Platform matches for these entity names (live biomedical KG @ EMBL-EBI):\n${lines.join("\n")}\n`;
}

// ── Claude reasoning calls ───────────────────────────────────────────────────
async function claudeExtractEntities(dataset, openTargetsHits = null) {
  const otContext = formatOpenTargetsContext(openTargetsHits);
...
${otContext}
The Open Targets matches above (if present) are real biomedical entities (gene targets, diseases, drugs) from EMBL-EBI's Open Targets Platform. Use them as supporting evidence — when you extract an entity that has a strong Open Targets match, the entity is grounded in real data and you can mention the Open Targets ID in its description.
...
const TOOLS = {
  ingest_dataset: {
    description:
      "Incrementally ingest an AI-ready dataset into the knowledge graph. Queries the Open Targets Platform (EMBL-EBI's biomedical KG of target-disease-drug evidence) using dataset name + field names as seeds, then uses the matches to ground Claude-driven entity extraction. Infers relations, deduplicates using embedding-based similarity, and revises existing edges if new evidence warrants it.",
    inputSchema: {
      type: "object",
...
async function executeTool(name, args) {
  switch (name) {
    ...
    case "ingest_dataset": {
      ...
      // Open Targets Platform seed lookup (bioinformatics tool).
      // Use dataset name + fields as seed terms to find real biomedical
      // entities (genes, diseases, drugs) that may appear in this dataset.
      // The matches are passed to Claude as grounding evidence during
      // entity extraction. Fail-soft: any error here proceeds with
      // Claude-only extraction.
      let openTargetsHits = null;
      try {
        const seeds = [
          dataset.name,
          ...(dataset.fields || []).slice(0, 5),
        ].filter(s => typeof s === "string" && s.trim().length > 2);

        if (seeds.length > 0) {
          log.push(`Calling Open Targets Platform for ${seeds.length} seed term(s)...`);
          openTargetsHits = await openTargetsResolveEntities(seeds, 3);
          const seedsWithHits = Object.values(openTargetsHits).filter(h => h.length > 0).length;
          log.push(`Open Targets: ${seedsWithHits}/${seeds.length} seeds matched real biomedical entities`);
        }
      } catch (err) {
        log.push(`Open Targets lookup failed (${err.message}) — proceeding with Claude-only extraction`);
        openTargetsHits = null;
      }

      // Extract entities via Claude (with Open Targets context if available)
      log.push("Calling Claude: extract entities...");
      const extracted = await claudeExtractEntities(dataset, openTargetsHits);
      const newNodes = [];
