...
async function claudeHarmonizeFields(dataset) {
...
const TOOLS = {
  harmonize_dataset: {
    description: "Harmonize a structured metadata description into an AI-ready, FAIR-scored dataset. Applies Darwin Core ontology mapping, unit normalization, missing value strategies, field renaming with flags, and automatically hands off to Module 3.",
    inputSchema: {
      type: "object",
...
async function executeTool(name, args) {
  switch (name) {
    ...
    case "harmonize_dataset": {
      ...
      // Step 1: Field harmonization + ontology mapping
      log.push("Calling Claude: field harmonization + Darwin Core mapping...");
      const harmonization = await claudeHarmonizeFields(args);
