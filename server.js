...
// ── PubTator 3.0 API helpers ──────────────────────────────────────────────────
// PubTator 3.0 is an NCBI literature resource that pre-annotates ~36M PubMed
// abstracts and ~6M PMC full-text articles with entity (gene, disease,
// chemical, species, variant) and relation annotations using the AIONER deep
// learning model. We use it for two purposes:
//   1. Broaden recall — its semantic search often surfaces papers that raw
//      PubMed keyword search misses.
//   2. Enrich abstracts with structured entity tags so Claude has stronger
//      context when extracting legacy-database mentions.
//
// Reference: Wei et al., "PubTator 3.0: an AI-powered Literature Resource for
// Unlocking Biomedical Knowledge", Nucleic Acids Research, 2024.
// API base: https://www.ncbi.nlm.nih.gov/research/pubtator3-api/
const PUBTATOR_BASE = "https://www.ncbi.nlm.nih.gov/research/pubtator3-api";

/**
 * Search PubTator 3.0 for relevant PMIDs given free-text queries.
 * Returns the union (OR) of PMIDs across all queries, capped at maxResults.
 * Designed to fail soft — on any error returns [] so the pipeline falls back
 * to vanilla PubMed search.
 */
async function pubtatorSearch(queries, maxResults = 100) {
  const allPmids = new Set();

  for (const q of queries) {
    if (allPmids.size >= maxResults) break;
    const url = `${PUBTATOR_BASE}/search/?text=${encodeURIComponent(q)}&page=1`;
    try {
      const res = await fetch(url, {
        headers: { "User-Agent": "kg-module1/1.0 (research)" },
        signal: AbortSignal.timeout(15000)
      });
      if (!res.ok) {
        process.stderr.write(`[kg-module1] PubTator search HTTP ${res.status} for "${q}" — skipping\n`);
        continue;
      }
      const data = await res.json();
      // PubTator search returns { results: [{ pmid, ... }, ...], ... }
      // Field shape can vary; defensively extract PMIDs from common locations.
      const results = data.results || data.PubTator3 || [];
      for (const r of results) {
        const pmid = String(r.pmid || r.PMID || r.id || "").replace(/\D/g, "");
        if (pmid) allPmids.add(pmid);
        if (allPmids.size >= maxResults) break;
      }
      process.stderr.write(`[kg-module1] PubTator: query "${q.slice(0, 40)}" → ${results.length} hits\n`);
    } catch (err) {
      process.stderr.write(`[kg-module1] PubTator search error for "${q}": ${err.message} — skipping\n`);
    }
  }

  return [...allPmids].slice(0, maxResults);
}

/**
 * Fetch PubTator 3.0 BioC-JSON annotations for a list of PMIDs.
 * Returns a map of pmid → { entities: [{type, name, identifier}], relations: [...] }.
 * Falls back to {} on any error — annotations are an enhancement, not required.
 */
async function pubtatorAnnotations(pmids) {
  const out = {};
  if (!pmids || pmids.length === 0) return out;

  // Batch in groups of 50 — endpoint accepts comma-separated PMIDs
  const batchSize = 50;
  for (let i = 0; i < pmids.length; i += batchSize) {
    const batch = pmids.slice(i, i + batchSize);
    const url =
      `${PUBTATOR_BASE}/publications/export/biocjson` +
      `?pmids=${batch.join(",")}`;
    ...
    try {
      ...
      if (!res.ok) {
        process.stderr.write(`[kg-module1] PubTator annotate HTTP ${res.status} — batch skipped\n`);
        continue;
      }

      // BioC-JSON can come as one JSON object per line OR as a single array.
      ...
      try {
        ...
        for (const line of text.split("\n")) {
          ...
          if (!trimmed) continue;
          try { docs.push(JSON.parse(trimmed)); } catch { /* skip */ }
        }
      }

      for (const doc of docs) {
        const pmid = String(doc.id || doc.pmid || "").replace(/\D/g, "");
        if (!pmid) continue;
        const entities = [];
        for (const passage of (doc.passages || [])) {
          for (const ann of (passage.annotations || [])) {
            const type = ann.infons?.type || ann.infons?.Type || "Unknown";
            const identifier = ann.infons?.identifier || ann.infons?.Identifier || null;
            const name = ann.text || "";
            if (name) entities.push({ type, name, identifier });
          }
        }
        ...
        out[pmid] = { entities: dedupedEntities };
      }
    } catch (err) {
      process.stderr.write(`[kg-module1] PubTator annotate error: ${err.message} — batch skipped\n`);
    }
  }
...
/**
 * Format PubTator entity annotations as a compact string for inclusion in
 * Claude's extraction prompt. Returns an empty string if there are no
 * annotations so the prompt cleanly degrades to PubMed-only behavior.
 */
function formatPubTatorContext(annotations) {
  if (!annotations || !annotations.entities || annotations.entities.length === 0) {
    return "";
  }
  // Group by entity type, cap each group to avoid prompt bloat
  const byType = {};
  for (const e of annotations.entities) {
    const k = e.type;
    if (!byType[k]) byType[k] = [];
    if (byType[k].length < 12) byType[k].push(e.name);
  }
  const lines = [];
  for (const [type, names] of Object.entries(byType)) {
    lines.push(`  ${type}: ${names.join("; ")}`);
  }
  return `\nPubTator 3.0 entity annotations for this paper (NCBI pre-tagged):\n${lines.join("\n")}\n`;
}
...
async function claudeExtractDatabases(paper, yearCutoff, pubtatorAnn = null) {
  const ptContext = formatPubTatorContext(pubtatorAnn);
...
Your task: Identify every database that is EXPLICITLY named and cited in this paper. Do not infer databases from context — only include ones clearly named. The PubTator entity annotations above (if present) are pre-extracted by NCBI's AIONER model — use them as supporting context for the paper's domain, but DO NOT treat tagged entities as database names. Database names usually appear in the abstract text itself.
...
const TOOLS = {
  search_pubmed: {
    description: "Search PubMed AND PubTator 3.0 (NCBI's AI-annotated literature index) with one or more free-text keyword queries, extract legacy database references from matching papers, score them with FAIR principles, and add them to the discovery registry. PubTator 3.0 provides entity-level annotations (genes, diseases, chemicals, species) over ~36M PubMed abstracts that supplement raw PubMed keyword search and improve database extraction accuracy. Legacy databases must meet all three criteria: pre-relational structure, no current MCP/API access, and evidence of being old or abandoned.",
    inputSchema: {
      type: "object",
      required: ["queries"],
      properties: {
...
{"disambiguated": ["query 1", "query 2"]}`;
      ...
      // Search PubMed (primary source) and PubTator 3.0 (broaden recall +
      // get pre-tagged entity annotations for each paper).
      let papers;
      try {
      ...
      // ── PubTator 3.0 augmentation ──────────────────────────────────────────
      // Fail-soft: any error here logs a warning and the pipeline continues
      // with PubMed-only results.
      let pubtatorMap = {};
      const pubmedPmids = new Set(papers.map(p => p.pmid));
      try {
        const ptPmids = await pubtatorSearch(finalQueries, maxResults);
        const newPtPmids = ptPmids.filter(p => !pubmedPmids.has(p));
        log.push(`PubTator 3.0 returned ${ptPmids.length} PMIDs (${newPtPmids.length} new beyond PubMed)`);

        // Fetch metadata for the brand-new PubTator-only PMIDs via efetch so
        // they enter the pipeline with the same shape as PubMed papers.
        if (newPtPmids.length > 0) {
          const cap = Math.min(newPtPmids.length, Math.max(0, maxResults - papers.length));
          if (cap > 0) {
            const fetchUrl = `${PUBMED_BASE}/efetch.fcgi?db=pubmed&id=${newPtPmids.slice(0, cap).join(",")}&retmode=xml&rettype=abstract`;
            const ptFetch = await fetch(fetchUrl);
            if (ptFetch.ok) {
              const xml = await ptFetch.text();
              const extra = parsePubMedXML(xml).filter(p =>
                yearCutoff ? (p.year > 0 && p.year <= yearCutoff) : true
              );
              papers.push(...extra);
              log.push(`  Fetched metadata for ${extra.length} PubTator-only papers`);
            }
          }
        }

        // Fetch entity annotations for ALL papers in scope
        const allPmids = papers.map(p => p.pmid).filter(Boolean);
        pubtatorMap = await pubtatorAnnotations(allPmids);
        const annotated = Object.keys(pubtatorMap).length;
        log.push(`PubTator 3.0 annotations retrieved for ${annotated}/${allPmids.length} papers`);
      } catch (err) {
        log.push(`PubTator 3.0 augmentation failed (${err.message}) — continuing with PubMed-only results`);
      }
      ...
      for (const paper of papers) {
        ...
        const ptAnn = pubtatorMap[paper.pmid] || null;
        if (ptAnn?.entities?.length) {
          log.push(`  PubTator entities: ${ptAnn.entities.length} (${[...new Set(ptAnn.entities.map(e => e.type))].join(", ")})`);
        }

        // Extract database references via Claude (with PubTator context if available)
        const extracted = await claudeExtractDatabases(paper, yearCutoff, ptAnn);
