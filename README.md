...
│    └─ PubTator 3.0 API (semantic search +        │
│        entity annotations — bioinformatics tool) │
...
Searches PubMed and PubTator 3.0 with one or more free-text keyword queries.
Results are combined with OR, so multiple queries broaden the search. Each
paper's abstract is analyzed by Claude, with PubTator 3.0 entity annotations
provided as supporting context, to identify explicitly named legacy databases.

**Bioinformatics tool: PubTator 3.0** (Wei et al., *Nucleic Acids Research*,
2024 — endpoints actively maintained at NCBI through 2025). PubTator 3.0
provides over one billion entity and relation annotations across approximately
36 million PubMed abstracts and 6 million PMC full-text articles, updated
weekly via NCBI's AIONER deep-learning pipeline. We use it to (1) broaden
recall beyond raw PubMed keyword search via its semantic search endpoint and
(2) enrich each paper's prompt with pre-extracted gene/disease/chemical/species
tags so Claude has stronger domain context when extracting database mentions.
The integration follows the LLM-augmentation pattern described in the PubTator
3.0 paper.

**API endpoints used (no auth required):**
- `GET /research/pubtator3-api/search/?text=...&page=1` — semantic PMID search
- `GET /research/pubtator3-api/publications/export/biocjson?pmids=...` — entity annotations
